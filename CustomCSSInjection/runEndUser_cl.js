...
checkNodes = (nodes, pb) => {
    nodes.forEach(node => {
        ...
        if (endUser && endUser.shadowRoot && !inserted) {
            ...
            // 3dot temp fix
            customEUStyle += `.qui-action-menu-container{ bottom: 15px !important;}`;
...
function runEndUser(pb) {
    ...
    if (endUser) {
        ...
        mutation = new MutationObserver(mutationList => {
            mutationList.forEach((mutation) => checkNodes(mutation.addedNodes, pb));
        });
        ...
        if (cb) {
            window.customElements.whenDefined('an-filter-builder-modal').then(function () {
                ...
                for (let index = 0; index < anFilterModal.length; index++) {
                    ...
                    // StyleSheet
                    if (typeof widgetCSSurl === 'undefined') {
                        widgetCSSurl = [];
                    }
