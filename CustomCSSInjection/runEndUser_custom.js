        ...
        if (endUser.shadowRoot && !inserted) {
            ...
            loadCSS(endUser, widgetCSSurl, 0).then(() => {
                ...
                window.customElements.whenDefined('an-panel').then(function () {
                    ...
                    if (panels.length > 0) {
                        document.querySelector('qrvey-end-user').classList.remove("loading-qv-end-user");
...
function runEndUser() {
    ...
    const endUser = document.querySelector('qrvey-end-user');
    ...
    if (endUser) {
        ...
        mutation = new MutationObserver(mutationList => {
            mutationList.forEach((mutation) => checkNodes(mutation.addedNodes));
        });
...
window.addEventListener('openFilterBuilder', function (event) {
    setTimeout(() => {
        ...
        if (endUser) {
            ...
            window.customElements.whenDefined('an-filter-builder').then(function () {
                let endUser = document.querySelector('qeu-end-user');
                let anFilterModal = endUser.shadowRoot.querySelectorAll('an-filter-builder-modal');
                let anFilterBuilder = anFilterModal[0].shadowRoot.querySelectorAll('an-filter-builder');
                if (anFilterBuilder.length > 0) {
                    // Styles to anFilterBuilder
                    for (let index = 0; index < anFilterBuilder.length; index++) {
                        const element = anFilterBuilder[index];
